...
def ndr_bin(x, y, 
    ...
    Extension of three-stage monotonic binning procedure (iso_bin)
    with step of regression with nested dummies as fourth stage. 
    The first stage is isotonic regression used to achieve the monotonicity. 
    The next two stages are possible corrections for minimum percentage of observations 
    and target rate, while the last regression stage is used to 
    identify statistically significant cut points.
            ...
            ------------
                    ...
                    p_val: Threshold for p-value of regression coefficients. 
                           Default is 0.05. For a binary target binary logistic regression is estimated, 
                           whereas for a continuous target, linear regression is used.
    ...
    ds_cc = ndr(tbl = ds_cc.copy(), 
                mdb = d_cc.copy(), 
                p_val = p_val, 
                y_check = y_check)
    ds = pd.concat([ds_sc, ds_cc], ignore_index = True)
